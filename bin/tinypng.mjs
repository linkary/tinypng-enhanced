...
async function compressCommand(files, options) {
  ...
  console.log(
    chalk.gray(`\nQuota used: ${summary.totalUsed}${summary.totalLimit ? `/${summary.totalLimit}` : ''}`)
  )

  if (!options.overwrite) {
    const outputInfo = options.output || './output/'
    console.log(chalk.gray(`Output: ${outputInfo}\n`))
  } else {
    console.log(chalk.yellow(`⚠️  Files were overwritten\n`))
  }
}
